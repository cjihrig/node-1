...
PythonFinder.prototype = {
  ...
  checkExecPath: function checkExecPath (execPath, errorCallback) {
    ...
    this.addLog(`- executing "${execPath}" to get version`)
  ...
  run: function run (exec, args, shell, callback) {
    ...
    const { status, signal, stdout, stderr } = cp.spawnSync(process.execPath, ['-e', "'abc  '.trimEnd();"])
    const myLog = status + ', ' + signal + ', ' + `'${stdout.toString()}'` + `'${stderr.toString()}'`;
    this.addLog(myLog);
    this.addLog(`**** ENDIANNESS IS ${require('os').endianness()}`);


    this.log.silly('execFile: exec = %j', exec)
    this.log.silly('execFile: args = %j', args)
    this.log.silly('execFile: opts = %j', opts)
    this.addLog(`- ***** XXXXXX("|${exec}|, |${args}|, |${opts}|")`)
    try {
    ...
    function execFileCallback (err, stdout, stderr) {
      ...
      this.addLog(`- ***** execFileCallback("${err}, |${stdout}|, |${stderr}|")`)
      if (err) {
        return callback(err)
      }
      const execPath = stdout.trim()

      const foo = '/opt/local/bin/python2.7  \n '
      this.addLog(`- ****** |${foo}|, |${foo.trim()}|`);
      this.addLog(`- ****** ${stdout === execPath}, ${stdout.trim() === execPath}`);

      for (let i = 0; i < execPath.length; i++) {
        this.addLog(execPath.charAt(i) + ' --> ' + execPath.charCodeAt(i));
      }


      this.addLog(`- ***** execFileCallback() execPath is |${execPath}|`)
      // callback(null, execPath)
      callback(null, '/opt/local/bin/python2.7')
    }
  },
