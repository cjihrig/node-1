...
Server.prototype.listen = function() {
  ...
  if (arguments.length === 0 || typeof arguments[0] === 'function') {
      ...
      if (typeof h.port === 'number' || typeof h.port === 'string' ||
        ...
        if (typeof h.port !== 'undefined' && !isLegalPort(h.port))
          throw new RangeError('"port" option should be >= 0 and < 65536: ' +
                               h.port);
        if (h.host)
          listenAfterLookup(h.port | 0, h.host, backlog, h.family, h.exclusive);
        else
          listen(self,
                 null,
                 h.port | 0,
    ...
    listenAfterLookup(port, arguments[1], backlog, 0);
  }

  function listenAfterLookup(port, address, backlog, family, exclusive) {
    const options = {family};

    require('dns').lookup(address, options, function(err, ip, addressType) {
      if (err) {
        self.emit('error', err);
      } else {
