...
const schedulers = {
  [SCHED_NONE]: SharedHandle,
  [SCHED_RR]: RoundRobinHandle,
  [SCHED_CUSTOM]: null
};
...
var scheduler = schedulers[schedulingPolicy];
...
cluster.setupMaster = function(options) {
  ...
  assert(schedulingPolicy === SCHED_NONE ||
         ...
         schedulingPolicy === SCHED_CUSTOM,
         `Bad cluster.schedulingPolicy: ${schedulingPolicy}`);

  if (schedulingPolicy === SCHED_CUSTOM) {
    schedulers[schedulingPolicy] = settings.scheduler;
  }

  scheduler = schedulers[schedulingPolicy];
